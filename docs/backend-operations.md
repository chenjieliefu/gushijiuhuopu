...
这份说明覆盖后端可独立完成的运行保护、备份恢复、部署检查。服务器和项目子域名信息已补充到 [部署资源交接](infrastructure-handoff.md)，登录授权与部署待接手。新版前端播放器已完成本地交接，见 [前端放映验收](frontend-screening-handoff.md)。真实 AI 模块和共享部署仍待交付。
...
目前提供已验证的备份/恢复命令，服务器信息已提供，尚未接手登录并创建定时备份任务或异地备份位置。部署后应每日备份到受控位置，并至少做一次恢复演练。
...
王泽轩已按 [正式章节协议](formal-chapter.md) 完成播放器、素材接入及本地真实 HTTP 联调，运行方法和证据见 [前端交接](frontend-screening-handoff.md)。刘文浩可据此复测开始、逐段确认、刷新恢复、寄展确认及重置，再接手共享部署。

宋浩然需要交付异步模块入口、依赖、环境变量名称、供应商/模型标识、内部重试与 token 上限，以及正常/失败样例。刘文浩已有 `AI_PROVIDER_FACTORY` 入口和调用保护；实际凭据通过部署环境注入，不发到群聊、文档或仓库。

目标服务器和项目子域名见 [部署资源交接](infrastructure-handoff.md)。仍需补齐登录授权、现有服务盘点和备份位置，再配置 HTTPS、反向代理可信地址、真实 CORS、供应商密钥及共享链接。DNS 变更与证据以该交接文档为准，当前尚未部署游戏服务。
