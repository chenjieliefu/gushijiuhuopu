...
- [新版前端：连续放映、素材接入、启动与验收](docs/frontend-screening-handoff.md)
- [服务器、项目子域名与后端部署交接](docs/infrastructure-handoff.md)（含阿里云原截图和控制台核对证据）
- [前端素材与交互现状](docs/frontend-materials-status.md)（25 个 PNG 原图、v4 脚本、新版播放器待办）

新版前端已接入 v4 连续放映协议与现有美术，桌面/手机本地完整 HTTP 流程通过。在 `frontend/` 运行 `npm run dev:screening` 可体验；真实 AI、最终美术审定与公网部署仍待交付。旧探索样例保留为回归入口。
